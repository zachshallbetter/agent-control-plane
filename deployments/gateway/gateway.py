...
def project_context(owner, project):
    ...
    query="""query($owner:String!,$project:Int!){organization(login:$owner){projectV2(number:$project){number,title,updatedAt,items(first:100){nodes{id,type,isArchived,content{... on Issue{number,title,url,state,repository{nameWithOwner}} ... on PullRequest{number,title,url,state,repository{nameWithOwner}}}}}}}}"""
